...
if (!dbUrl) {
  console.log("ATLASDB_URL is missing in environment variables");
}
...
app.use((req, res, next) => {
  ...
  res.locals.wishlistCount =
    req.user && req.user.wishlist ? req.user.wishlist.length : 0;
  next();
});
...
app.use((err, req, res, next) => {
  if (err && err.name === "ValidationError") {
    if (req.originalUrl === "/listings" && req.method === "POST") {
      return res
        .status(400)
        .render("listings/new.ejs", { error: err, listing: req.body.listing });
    }
    ...
    if (req.originalUrl.startsWith("/listings/") && req.method === "PUT") {
      ...
      return res
        .status(400)
        .render("listings/edit.ejs", { error: err, listing: mergedListing });
    }
  }
  next(err);
});
